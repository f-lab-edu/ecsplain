...
def set_models(config):
    ...
    if "gpt-5" in config.openai_model:
        ...
        _LLM = ChatOpenAI(
            model=config.openai_model, api_key=api_key, temperature=config.temperature
        )

    emb_model = OpenAIEmbeddings(model=config.embedding_model, api_key=api_key)

    vector_store = Chroma(
        embedding_function=emb_model, persist_directory=str(BASE_DIR / Path(config.chroma_dir))
    )
